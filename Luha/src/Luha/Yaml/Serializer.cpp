...
namespace YAML {
    ...
    Node convert<ImVec2>::encode(const ImVec2& rhs)
    {
        Node node;
        node.push_back(rhs.x);
        node.push_back(rhs.y);
    ...
    bool convert<ImVec2>::decode(const Node& node, ImVec2& rhs)
    {
        if (!node.IsSequence() || node.size() != 2)
            return false;
    ...
    Node convert<ImVec4>::encode(const ImVec4& rhs)
    {
        Node node;
        node.push_back(rhs.x);
        node.push_back(rhs.y);
    ...
    bool convert<ImVec4>::decode(const Node& node, ImVec4& rhs)
    {
        if (!node.IsSequence() || node.size() != 4)
            return false;
    ...
    Emitter& operator<<(Emitter& out, const ImVec2& v)
    {
        out << Flow;
        out << BeginSeq << v.x << v.y << EndSeq;
        return out;
